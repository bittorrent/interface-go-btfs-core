...
func (tp *TestSuite) TestListSelf(t *testing.T) {
	...
	if keys[0].Path().String() != "/btns/"+coreiface.FormatKeyID(self.ID()) {
		t.Errorf("expected the key to have path '/btns/%s', got '%s'", coreiface.FormatKeyID(self.ID()), keys[0].Path().String())
	}
}
...
func verifyIPNSPath(t *testing.T, p string) bool {
	t.Helper()
	if !strings.HasPrefix(p, "/btns/") {
		t.Errorf("path %q does not look like an BTNS path", p)
		return false
	}
	k := p[len("/btns/"):]
	c, err := cid.Decode(k)
	if err != nil {
		t.Errorf("failed to decode BTNS key %q (%v)", k, err)
		return false
	}
